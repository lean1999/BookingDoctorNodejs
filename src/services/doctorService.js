...
let saveDetailsInfoPatient = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data) {
        resolve({
        ...
        if (patientInfo) {
          ...
          if (data.isUpdateCode) {
            patientInfo.code = data.code;
          } else {
            patientInfo.doctorDiagnose = data.doctorDiagnose;
          }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.email || !data.doctorId || !data.patientId || !data.timeType) {
        ...
        //update
        console.log("da chay vo day dong 532")
...
let sendPrescription = (data) => {
  console.log("data form truong oc cho", data);
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.email || !data.doctorId || !data.patientId || !data.timeType) {
        resolve({
        ...
        if (appointment) {
          ...
          if (user) {
            console.log("user", user);
            data["doctorDiagnose"] = user.doctorDiagnose;
          }
...
module.exports = {
  ...
  saveDetailsInfoPatient: saveDetailsInfoPatient,
};
